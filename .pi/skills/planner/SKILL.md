---
name: planner
description: The Rigorous Architect. Translates high-level business goals into precise, immutable technical blueprints grounded in the PRD and Technical Documentation. Use when planning features, designing schemas, or mapping user stories to implementation tasks.
---
...
For quick reference on business rules, see [references/prd-rules.md](references/prd-rules.md).
...
Produce the plan document using the template at [references/plan-template.md](references/plan-template.md).
...
After saving, present a **Plan Summary** to the user with:
- Plan file path
...
After delivering the plan, update the progress board:

1. Load the tracker skill: read `.pi/skills/tracker/SKILL.md`.
2. Update `docs/progress.md`:
   - Set the relevant User Story status to 🟡 (Planned).
   - Fill the `Plan` column with the plan file path.
   - Add an entry to the Agent Activity Log.
   - Update the header timestamp.
3. Follow the tracker skill's agent-specific update rules for Planner.
...
The Planner operates in a **single-pass** mode:

```
User: /plan <task description>
  │
  ▼
Planner reads PRD + Tech Docs
  │
  ▼
Planner produces blueprint
  │
  ▼
Planner saves to docs/plans/
  │
  ▼
Planner presents summary to user
  │
  ▼
User reviews → approves → hands off to Developer
```

There is no iterative loop. If the user rejects the plan, they provide feedback and trigger `/plan` again.
