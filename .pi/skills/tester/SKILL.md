---
name: tester
description: The Quality Sentinel. Writes and enforces TDD tests for BikerFlow — validates business rules, financial formulas, and edge cases using PHPUnit within the Dev Container. Use when writing acceptance tests or verifying implementations.
---
...
For BikerFlow-specific test patterns and conventions, see [references/test-patterns.md](references/test-patterns.md).
...
**All test commands run through the sandbox skill.** Use the `docker exec devcontainer_app_1` prefix for every command.
...
**Trigger:** User invokes `/test <task-id>` and no tests exist for the feature yet.
...
5. Create the test file(s) following the patterns in [references/test-patterns.md](references/test-patterns.md).
...
8. Update the progress board (see Progress Tracking below).
...
**Trigger:** User invokes `/test <task-id>` and tests already exist for the feature.
...
2. If **all tests pass (GREEN)**:
   ...
   - Update the progress board to 🟣 (Tests Green).
3. If **any test fails (RED)**:
   ...
   - Update the progress board to reflect the failure state.
4. Update the progress board (see Progress Tracking below).
...
After completing either mode, update the progress board:

1. Load the tracker skill: read `.pi/skills/tracker/SKILL.md`.
2. Update `docs/progress.md`:
   - **RED mode:** Set User Story status to 🟡 (Planned), fill `Tests` column with test file paths. Add Activity Log entry noting "Tests written (RED — awaiting implementation)".
   - **GREEN mode:** Set User Story status to 🟣 (Tests Green). Add Activity Log entry noting "Tests confirmed GREEN".
   - **GREEN with failures:** Do NOT advance status. Add Activity Log entry noting "Tests RED — Developer fix required" with specific failures.
3. Follow the tracker skill's agent-specific update rules for Tester.
...
```
/plplan US-01 → Plan with acceptance criteria
       │
       ▼
/test US-01 (Mode 1: RED)
       │ Writes failing tests from AC-XX
       │ Reports: "5 tests written, all RED — ready for Developer"
       │ Updates progress
       ▼
/develop US-01 → Developer implements to make tests pass
       │
       ▼
/test US-01 (Mode 2: GREEN)
       │ Runs existing tests
       │ Reports: "5/5 GREEN, 0 regressions — ready for Validator"
       │    OR: "3/5 GREEN, 2 failures — back to Developer"
       │ Updates progress
       ▼
/validate US-01 → Validator audits
```
