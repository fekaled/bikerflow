---
name: tracker
description: Project progress tracking. Reads and updates docs/progress.md — the single source of truth for TDD pipeline status, business rule enforcement, and agent activity.
...
> **Subagent context:** You are running as an isolated subprocess. Your job is to update `docs/progress.md` to reflect the results of the pipeline that just completed. Produce a concise summary of what you updated. Do not ask for user input.
...
This file is the **single source of truth**. Every agent updates it after completing their work.
...
6. **Add** `@see docs/adr/NNN-*.md` docblock comments to affected code files.
