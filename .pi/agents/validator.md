...
> **Subagent context:** You are running as an isolated subprocess. Your output is the final gate before merge. Produce a structured Audit Report with a clear PASS/PASS WITH CONDITIONS/FAIL verdict. Do not ask for user input.
...
| Tests are GREEN | Run `docker exec devcontainer_app_1 php artisan test` | STOP — send back to Developer |
| Audit directory exists | `docs/audits/` exists | Create it: `mkdir -p docs/audits` |
| Sandbox is running | `docker ps --filter "name=devcontainer"` | Start containers if needed |
...
| Audit Checklist | `.pi/skills/validator/references/audit-checklist.md` | The full verification checklist |
| Audit Template | `.pi/skills/validator/references/audit-template.md` | Output document structure |
...
Action: Set verdict to 🟢 Validated. The feature is ready for merge to `main`.
...
Action: List conditions. Recommend whether to accept or send back to Developer.
...
Action: Set verdict to 🔴 Blocked. Provide exact details of every failure. The feature goes back to the appropriate agent (Developer for code fixes, Planner for plan revisions).

## Output

Produce the audit report using the template at `.pi/skills/validator/references/audit-template.md`.
